...
func (r *UserDataSourceModel) RefreshFromSharedUserView(ctx context.Context, resp *shared.UserView) diag.Diagnostics {
	...
	if resp.UserActorObjectPermissions != nil {
		r.Delete = types.BoolPointerValue(resp.UserActorObjectPermissions.Delete)
		r.Edit = types.BoolPointerValue(resp.UserActorObjectPermissions.Edit)
		if len(resp.UserActorObjectPermissions.Extra) > 0 {
			r.Extra = make(map[string]types.Bool, len(resp.UserActorObjectPermissions.Extra))
			for key, value := range resp.UserActorObjectPermissions.Extra {
				r.Extra[key] = types.BoolValue(value)
			}
		}
		r.Read = types.BoolPointerValue(resp.UserActorObjectPermissions.Read)
	}
