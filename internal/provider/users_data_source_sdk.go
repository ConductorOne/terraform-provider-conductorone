	...
	if resp != nil {
		...
		if resp.List != nil {
			...
			for _, listItem := range resp.List {
				...
				if listItem.UserActorObjectPermissions == nil {
					list.UserActorObjectPermissions = nil
				} else {
					list.UserActorObjectPermissions = &tfTypes.UserActorObjectPermissions{}
					list.UserActorObjectPermissions.Delete = types.BoolPointerValue(listItem.UserActorObjectPermissions.Delete)
					list.UserActorObjectPermissions.Edit = types.BoolPointerValue(listItem.UserActorObjectPermissions.Edit)
					if len(listItem.UserActorObjectPermissions.Extra) > 0 {
						list.UserActorObjectPermissions.Extra = make(map[string]types.Bool, len(listItem.UserActorObjectPermissions.Extra))
						for key, value := range listItem.UserActorObjectPermissions.Extra {
							list.UserActorObjectPermissions.Extra[key] = types.BoolValue(value)
						}
					}
					list.UserActorObjectPermissions.Read = types.BoolPointerValue(listItem.UserActorObjectPermissions.Read)
				}
				list.UserID = types.StringPointerValue(listItem.UserID)
