...
func (r *IntegrationSentryResourceModel) ToCreateSDKType() *shared.ConnectorServiceCreateDelegatedRequest {
...
func (r *IntegrationSentryResourceModel) ToUpdateSDKType() *shared.Connector {
	userIds := make([]string, 0)
	for _, userIdsItem := range r.UserIds {
		userIds = append(userIds, userIdsItem.ValueString())
	}
...
func (r *IntegrationSentryResourceModel) RefreshFromGetResponse(resp *shared.Connector) {
	...
	if resp.Config != nil && *resp.Config.AtType == envConfigType {
		if config, ok := resp.Config.AdditionalProperties.(map[string]interface{}); ok {
			if v, ok := config["sentry_org_slug"]; ok {
				r.SentryOrgSlug = types.StringValue(v.(string))
			}

			if v, ok := config["sentry_token"]; ok {
